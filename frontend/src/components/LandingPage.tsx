...
const COLS = 14;
const ROWS = 8;
const CELL = 56; // Smaller cells for embedded preview
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <header className="border-b border-[#00ff00]/20 px-6 py-4 flex items-center justify-between">
        ...
        {/* Mobile menu button */}
        <button className="md:hidden text-[#00ff00]">
      ...
      <section className="py-12 md:py-16 text-center px-4">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4">
          <button
            onClick={onViewLive}
            className="w-full md:w-auto px-8 py-3 bg-[#00ff00] text-black font-bold rounded-lg hover:bg-[#00cc00] transition-colors flex items-center justify-center gap-2"
          >
            VIEW LIVE
          ...
          <Link
            href="/docs"
            className="w-full md:w-auto px-8 py-3 border border-gray-600 text-white font-bold rounded-lg hover:bg-gray-900 transition-colors"
          >
            READ DOCS
          </Link>
        </div>
      ...
      {/* ========== LIVE GAME PREVIEW (Desktop) ========== */}
      <section className="hidden md:block px-6 pb-8">
      ...
      {/* ========== MOBILE PREVIEW (GIF Placeholder) ========== */}
      ...
      <section className="px-4 md:px-6 pb-12">
        <div className="max-w-4xl mx-auto">
          <div className="grid grid-cols-3 gap-4 md:gap-8">
            <div className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-white">824</div>
              <div className="text-gray-500 text-xs md:text-sm uppercase tracking-wider">ACTIVE BOTS</div>
            </div>
            <div className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-[#00ff00]">$12.4k</div>
              <div className="text-gray-500 text-xs md:text-sm uppercase tracking-wider">PRIZE POOL</div>
            </div>
            <div className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-white">14<span className="text-lg">ms</span></div>
              <div className="text-gray-500 text-xs md:text-sm uppercase tracking-wider">TICK RATE</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ========== MOBILE BOTTOM NAV ========== */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0a0a0a] border-t border-gray-800 pb-safe">
        <div className="flex justify-around py-3">
          <button className="flex flex-col items-center gap-1 text-[#00ff00]">
            <span className="text-xl">🎮</span>
            <span className="text-[10px] font-bold">ARENA</span>
          </button>
          <Link href="/lobby" className="flex flex-col items-center gap-1 text-gray-500">
            <span className="text-xl">⚔️</span>
            <span className="text-[10px] font-bold">LOBBY</span>
          </Link>
          <Link href="/leaderboard" className="flex flex-col items-center gap-1 text-gray-500">
            <span className="text-xl">📊</span>
            <span className="text-[10px] font-bold">BOARD</span>
          </Link>
          <Link href="/register" className="flex flex-col items-center gap-1 text-gray-500">
            <span className="text-xl">🤖</span>
            <span className="text-[10px] font-bold">MY BOT</span>
          </Link>
        </div>
      </nav>
      ...
      {showRegisterModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-[#111] border border-[#00ff00]/30 rounded-xl p-6 max-w-md w-full">
            <h2 className="text-2xl font-bold mb-4">Register Your Bot</h2>
            <p className="text-gray-400 mb-6">Coming soon...</p>
            <button
              onClick={() => setShowRegisterModal(false)}
              className="w-full py-3 bg-[#00ff00] text-black font-bold rounded-lg"
            >
              Close
            </button>
          </div>
